...
export default function App() {
  ...
  }, [isDragging, draggedShape, draggedHandIndex, hoveredRow, hoveredCol, board, boardSize]);
  ...
  // 3) Bomb Mode activation and targeting resolver (Cost: 200)
  const handleToggleBombMode = () => {
  ...
  const handleExecuteBombAt = (r: number, c: number) => {
    ...
    // Clear cells in a 3x3 quadrant centering on clicking coordinate (r, c)
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        const tr = r + dr;
        const tc = c + dc;
        if (tr >= 0 && tr < size && tc >= 0 && tc < size) {
          nextBoard[tr][tc] = { filled: false, color: '', styleId: '' };
        }
      }
    }
    ...
    // Players can use Bomb powerups to rescue themselves from game over traps, so re-check locks!
    setIsGameOver(false);
    evaluateGameOverTraps(nextBoard, hand);
  };
    ...
    >
      ...
      <footer className="relative z-10 w-full pb-2">
        <PowerUps
          ...
          onToggleBombMode={handleToggleBombMode}
